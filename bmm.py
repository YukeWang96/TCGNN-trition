...
# torch.manual_seed(0)
nodes = int(32768 / 32) * 32
a = torch.randn((nodes, nodes), device='cuda', dtype=torch.float)
b = torch.randn((nodes, 16), device='cuda', dtype=torch.float)
...
dur = (time.perf_counter() - start)/rounds * 1e3
print("Time (ms): {:.3f}".format(dur))
